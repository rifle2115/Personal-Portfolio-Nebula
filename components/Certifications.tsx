    ...
    return (
        ...
        >
            <div className="w-full max-w-4xl">
                <motion.h2
                    className="text-2xl sm:text-3xl md:text-4xl font-bold mb-6 sm:mb-8 md:mb-14 text-center text-white"
                ...
                >
                    {certifications.map((cert, index) => (
                        <motion.div
                            ...
                            className={`group relative bg-white/[0.03] backdrop-blur-sm border border-white/10 rounded-xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-start md:items-center gap-3 sm:gap-4 md:gap-5 transition-all duration-300 hover:-translate-y-0.5 ${cert.borderColor} ${cert.glowColor}`}
                        >
                            {/* Logo + Content row on mobile */}
                            <div className="flex items-start gap-3 sm:contents">
                                {/* Logo */}
                                <div
                                    className={`flex-shrink-0 w-11 h-11 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg bg-gradient-to-br ${cert.color} flex items-center justify-center border border-white/10`}
                                >
                                    {cert.logo}
                                </div>

                                {/* Content - inline with logo on mobile */}
                                <div className="flex-1 min-w-0 sm:hidden">
                                    <h3 className="text-sm font-semibold text-white group-hover:text-purple-300 transition-colors duration-300 leading-tight">
                                        {cert.title}
                                    </h3>
                            ...
                            {/* Content - tablet+ */}
                            <div className="flex-1 min-w-0 hidden sm:block sm:text-left">
                                ...
                                <p className="text-xs md:text-sm text-gray-500 mt-2 md:mt-1.5 leading-relaxed line-clamp-2">
                                    {cert.description}
                                </p>
                            </div>

                            {/* Description - mobile only (below header) */}
                            <p className="text-xs text-gray-500 leading-relaxed line-clamp-3 sm:hidden -mt-1">
                                {cert.description}
                            </p>
                            ...
                            <a
                                ...
                                className="flex-shrink-0 w-full sm:w-auto flex items-center justify-center gap-1.5 px-4 py-2.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-gray-400 transition-all duration-300 hover:bg-purple-500/20 hover:border-purple-500/40 hover:text-purple-300 active:bg-purple-500/30 hover:shadow-[0_0_15px_rgba(168,85,247,0.3)]"
                            >
                                Verify
